class hashTable:
    ...
    def insert(self, record):
        ...
        if self.table[position] == None: 
            ...
            print("Collision has occured for " + record.get_name() + "'s phone number at position finding new Position.")
...
class doubleHashTable:
    ...
    def doubleHashing(self, record): 
        ...
        i = 1
        while i <= limit:
            newPosition = (self.h1(record.get_number()) + self.h2(record.get_number())) % self.size
    ...
    def search(self, record):
        ...
        if(self.table[position] != None):
            if(self.table[position].get_name() == record.get_name()):
                ...
                while i <= limit:
                    position = (self.h1(record.get_number()) + self.h2(record.get_number())) % self.size
                    self.comparisons += 1
                    if(self.table[position] != None):
                        if self.table[position].get_name() == record.get_name():
                            found = True
